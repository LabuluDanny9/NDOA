...
import { toGuestTableInsert } from "@/lib/api/serializers"
...
export async function GET(request: NextRequest, context: RouteContext) {
  return withApiErrors(async (requestId) => {
    ...
    const { data, error, count } = await supabase.from("guest_tables").select("*", { count: "exact" }).eq("wedding_id", weddingId).order(sort, { ascending }).range(pagination.from, pagination.to)
...
export async function POST(request: NextRequest, context: RouteContext) {
  return withApiErrors(async (requestId) => {
    const { supabase } = await requireApiContext()
    const weddingId = parseWithSchema(uuidSchema, (await context.params).weddingId)
    const input = parseWithSchema(guestTableSchema, await readJsonBody(request))
    const { data, error } = await supabase.from("guest_tables").insert(toGuestTableInsert(input, weddingId)).select("*").single()
    if (error) throw apiErrorFromSupabase(error)
